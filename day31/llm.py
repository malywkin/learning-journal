"""
День 31 — провайдер модели: DeepSeek основной, локальный qwen3.5 (LM Studio) запасной.

Это §14 конспекта (Retry → Fallback → Human-in-the-Loop) в миниатюре: сначала пробуем
основную модель; если она упала (сеть/500), падаем на локальную. Обе — по одному
OpenAI-совместимому SDK (развязка провайдера с Дня 24: меняется только base_url+ключ+имя).

Ключ читаем из .env (не хардкодим). Клиент async — роутер и веб-приложение асинхронные.
"""
import os
...
def _client(p: dict) -> AsyncOpenAI:
    if p["name"] not in _clients:
        _clients[p["name"]] = AsyncOpenAI(base_url=p["base_url"], api_key=p["api_key"], timeout=60)
    ...
    for p in _providers():
        try:
            r = await _client(p).chat.completions.create(
                ...
                temperature=0, max_tokens=1400, **kw)
            return r.choices[0].message, p["name"]
        except Exception as e:
            last_err = f"{p['name']}: {type(e).__name__}"
            continue
    # Все провайдеры недоступны — human-in-the-loop (§14): честно говорим.
    class _Stub:
